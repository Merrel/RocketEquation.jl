...
println("\n=============================================\nBegin Mission")
df = DataFrame(
    Event = Type[], Start=String[], End=String[], ΔV = [], VehicleName=String[], VehicleGross=[], ActiveVehicleProp = []
)
...
push!(df, [typeof(action) action.src action.dst action.dV name(vehicle) gross(vehicle) propellant(vehicle)])
...
push!(df, [Staging last(df).End last(df).End 0m/s name(Starship) gross(Starship) propellant(Starship)])
push!(df, [Staging last(df).End last(df).End 0m/s name(ESM_Orion) gross(ESM_Orion) propellant(ESM_Orion)])

# 3b. Crew transfer
(Starship, ESM_Orion) = transfer_crew!(ESM_Orion, Starship)
push!(df, [CrewTransfer last(df).End last(df).End 0m/s name(Starship) gross(Starship) propellant(Starship)])
push!(df, [CrewTransfer last(df).End last(df).End 0m/s name(ESM_Orion) gross(ESM_Orion) propellant(ESM_Orion)])
